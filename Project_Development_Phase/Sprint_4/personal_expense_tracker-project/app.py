...
lu = cursor.fetchone()
if not lu:
    cursor.execute('''INSERT INTO user VALUES (0,'admin@admin','admin','admin',0);''')
...
def index():
    if current_user.is_authenticated:
        ...
        if totalBudgetNow<totalExpenseNow:
            sendgridmail(current_user.email,"You have Exceeded Your budget")
                    
        return render_template('index.html', expenses=all_expenses,email=current_user.email ,name=current_user.name,totalExpense=totalExpense,totalExpenseNow=totalExpenseNow,totalBudgetNow=totalBudgetNow,current_month=current_month)
